...
class oh58d {
    ...
    static createButtonCommands(waypoints) {
        ...
        for (const waypoint of waypoints) {
            ...
            let convertedWaypoint = `${zone} ${squareId} ${easting.slice(0, 4)} ${northing.slice(0, 4)}`;
            for (let i = 0; i < convertedWaypoint.length; i++) {
                if (convertedWaypoint.charAt(i) !== " ") {
                    this.#codesPayload.push( // enter each digit of MGRS into scratchpad
                        createButtonPress(14, this.#oh58dkeycodes[convertedWaypoint.charAt(i).toLowerCase()], this.#delay100),
                    );
                }
            }
